...
const IndustryProjectForm = () => {
  const [formData, setFormData] = useState({
    // Section 1 — Contact
    contactName: '', designation: 'Industry Professional', department: '', employeeId: '',
    email: '', contactNumber: '', linkedIn: '',
    // Industry-specific contact fields
    companyName: '', companyWebsite: '', companySize: '11-50',
    // Section 2 — Overview
    ...
    mentorAvailability: '3-5 Hours', collaborationMode: 'Hybrid', repoLink: '',
    // Section 6 — Outcomes
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        proposerName: formData.contactName,
      ...
      await axios.post('http://localhost:5000/api/apply/project', payload);
      setStatus('success');
    } catch {
      setStatus('error');
    }
    ...
    return (
      <div className="min-h-[calc(100vh-72px)] bg-[#FAFAFA] flex items-center justify-center px-4">
        <div className="text-center max-w-lg">
          ...
          <p className="text-gray-600 font-medium mb-8 leading-relaxed">Thank you for proposing an Industry Project. We'll review your submission and reach out via email.</p>
    ...
    <div className="min-h-[calc(100vh-72px)] bg-[#FAFAFA] py-16 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-10">
          <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-yellow-50 text-yellow-800 text-xs font-black uppercase tracking-widest rounded-full mb-4 border border-yellow-200">
            🏢 FOR INDUSTRY
          </span>
          <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-3">
            <span className="text-gray-900">Industry Project </span>
            <span className="text-cusoc-red">Proposal</span>
          </h1>
          <p className="text-gray-600 max-w-2xl mx-auto font-medium leading-relaxed">
            Partner with Chandigarh University to bring your innovative ideas to life with
            <br />talented student developers during CUSoC 2026.
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-8 md:p-12">
          <div className="mb-10 inline-flex items-center gap-2 px-4 py-1.5 border border-yellow-200 rounded-full bg-yellow-50">
            <span className="text-yellow-700 text-xs font-black tracking-widest uppercase">✨ Industry Project Proposal</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            ...
            {/* SECTION 1 */}
            <div>
              <h2 className={sectionTitleClass}>SECTION 1 — Faculty/Industry Contact Information</h2>
              <div className="space-y-5">
                <div><label className={labelClass}>Faculty/Contact Name</label><input required name="contactName" onChange={handleChange} className={inputClass} /></div>
                <div>
                  <label className={labelClass}>Designation/Role</label>
                  <select name="designation" onChange={handleChange} className={inputClass}>
                    <option>Assistant Professor</option><option>Associate Professor</option><option>Professor</option><option>Industry Professional</option><option>Other</option>
                  </select>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div><label className={labelClass}>Department/Company</label><input required name="department" onChange={handleChange} className={inputClass} /></div>
                ...
                <div><label className={labelClass}>Official Email</label><input required type="email" name="email" placeholder="name@company.com" onChange={handleChange} className={inputClass} /></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div><label className={labelClass}>LinkedIn / Profile (Optional)</label><input name="linkedIn" onChange={handleChange} className={inputClass} /></div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-5">
                <div><label className={labelClass}>Problem Statement</label><textarea required name="problemStatement" rows="4" placeholder="Describe the business problem or challenge this project addresses." onChange={handleChange} className={inputClass} /></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <select name="mentorAvailability" onChange={handleChange} className={inputClass}>
                      <option>3-5 Hours</option><option>1-2 Hours</option><option>5+ Hours</option>
                    </select>
                  </div>
                  <div>
...
export default IndustryProjectForm;
